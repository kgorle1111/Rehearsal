...
- **[Resolved]** WS-API (`src/rehearsal/api/`, `src/rehearsal/store/`) had zero contract tests, despite BUILD.md's WS-API DoD explicitly requiring them. Found by test-writer during P5, dispatched as its own follow-up pass, done: 33 REST/WS tests (`tests/api/`) + 22 store tests (`tests/store/`), all 10 endpoints, session lifecycle, WS replay (from-start, live-tail, gap-resume). `httpx` added as a dev dependency (user-approved) for `starlette.testclient.TestClient`. Two real bugs found and fixed: `root_seed` overflowed a signed-64-bit SQLite column on ~50% of draws (uncaught 500 on session creation), and `GET /api/sessions/{id}` derived state from `fold(events)` — which only reconstructs durable checkpoints — so it reported `"armed"` immediately after a session was correctly started into `"source_speaking"`; now reads from the live cached FSM, falling back to the same fold()-derived value when none is cached.
