...
- **WS-API (`src/rehearsal/api/`, `src/rehearsal/store/`) has zero contract tests**, despite BUILD.md's WS-API DoD explicitly requiring them ("every endpoint has a contract test... DB migrations run clean... no network egress in the core loop (boundary test)"). This landed in the same out-of-band interruption window as the Ollama commit and never went through a proper WS-API dispatch — I only did lint/type/permission fixes on it, never built the contract-test suite its own DoD requires. Found by test-writer during P5. **This is the largest remaining gap in the build** — a full workstream's worth of test infrastructure (FastAPI TestClient setup, WS/SSE session-replay harness, SQLite schema fixtures), not a mechanical fix. Flagged for a decision, not fixed unilaterally.
